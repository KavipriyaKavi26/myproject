...
def get_student_timetable(student_id):
    ...
    cursor = conn.cursor()
    cursor.execute(f"SELECT * FROM STUDENT_TIMETABLE WHERE SID = '{student_id}'")
...
def challenge():
    ...
    if user == "Student":
        ...
        if len(cursor) == 0:
            ...
            timetable = get_student_timetable(id_entry.get())
            for slot in timetable:
                tk.Label(nw, text=f'{slot[0]} - {slot[1]}: {slot[2]}', font=('Consolas', 12)).pack()
            m.destroy()
            nw.mainloop()
